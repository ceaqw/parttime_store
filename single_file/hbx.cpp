...
{
	int i,j,k;
	for(i = 0;i<7;i++)  //遍历链表结构体检测 之下有循环来链表检测的都是对链表的遍历 
	{
		for(j=0;j<40;j++)
		{
			for(k =0;k<9;k++)
...
int admin_see()		//管理员信息查看
{
	...
	else if(cnt == 1)
	{
		printf("请输入所需维护的电脑编号：");
		scanf("%d",&num[0]);
		printf("请输入需要修改状态的时间段（星期1-7 时间段8-17）:");
	...
	else if(cnt == 3)		//保存某天信息 
	{
		...
		printf("当天使用状况以保存至程序目录下的count.txt文件中\n\n");
		fclose(fp);
		 return 1;
	}	
} 
...
int student_see()		//学生信息处理 
{
	...
	else if(cnt == 1)		//预约 
	{
		...
		printf("请输入机号：");
		scanf("%d",&num[0]);
		printf("请输入时间（周几1-7 开始时间段8-17 时间段数8-17 ）:");
	...
	{
		printf("请输入取消机号 周几 取消时间段个数 （数字空格隔开）：");
		scanf("%d %d %d",&num[0],&num[2],&num[1]);
		for(j=0;j<40;j++)				//遍历链表结构体检测 之下有循环来链表检测的都是对链表的遍历 
		{
			if(Lab[num[1]-1][j].No == num[0])
			{
				for(i=0;i<num[1];i++)
	...
	else if(cnt == 3)	//报错 
	{
		...
		printf("%s\n",error_msg);
		printf("输入成功，错误信息以保存至程序目录下error.txt文件中\n");
		fclose(fp);
		return 1;
	}
