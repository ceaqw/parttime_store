...
void denglu();
void SAVE_Info(Good stu[],int sum);//商品录入
...
int main()
{
	...
	do
	{
		...
		switch(choice)
		{
			...
			case 3: Revise_info(stu);break;  /*修改信息*/
			case 4: Delete_Info(stu);break;  /*删除信息*/
			case 5: Statistics_Info(stu);break;	/*统计信息*/
			case 6: Out_Good_Info(stu);break;		/*商品输出*/ 
			case 0: break;
		}
	}while(choice!=0);
...
{  
   ...
   for(i=0;i<MAX;i++)
   {
	   ...
	   printf("\n 请输入商品的入库时间：");
	   scanf("%s",stu[k].Good_time);
	   printf("\n 请输入商品的出库时间：");
	   scanf("%s",stu[k].Good_out_time);
	   k++;
	   printf("\n 请按1返回菜单或按0继续创建");
	   scanf("%d",&x);
	    if(x)
...
//	system("color 1d");
	for (i = 1; i < 4; i++)
	{
		...
		printf("\n        请输入你的密码");
		scanf("%s",userPWD);
		if ((strcmp(userName, "root") == 0 && (strcmp(userPWD, "123654") == 0)))/*验证用户名和密码*/
		{
			printf("\n        *登陆成功，显示主菜单*");
			return;
		}
