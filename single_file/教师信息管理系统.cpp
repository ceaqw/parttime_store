...
void init() {   //文件的初始化读取
	while (!feof(fp)) {
		fscanf(fp, "%d\t%s\t%s\t%s\t%s\t%s\t%s\n",cnt,LNode[i_number].gong, LNode[i_number].name,
...
void save(){
	...
	if(n==1){
		...
		scanf("%s",fp_name);
		Mg = fopen(fp_name,"w+");
		fprintf(fp,"记录编号\t教师工号\t姓名\t出生日期（年/月）\t学院\t住址\t联系方式\n"); 
...
void case1() {    
	...
	for(i=0; i<i_number; i++)  {          //判断输入数据的正误，即文件中是否已存在该信息
		if(strcmp(LNode[i].gong,LNode[i_number].gong)==0) {			//利用strcpy(a,b)函数比较字符串a与b是否相等，需调用string.h头文件。下面将多次用到，用法相同不再一一例举 
			flag=0;
		}
		} 
	if(flag==0){ 
	...
	scanf("%d",&t);
		if(t==1) {
			...
			if(flag==0){
				...
				char num1[MIN];
				scanf("%s",num1);
				strcpy(num1,LNode[n].name);
				puts("请输入修改后的出生日期：");
				char num2[MIN];
				scanf("%s",num2);
				strcpy(num2,LNode[n].brith);
				puts("请输入修改后的学院：");
				char num3[MIN];
				scanf("%s",num3);
				strcpy(num3,LNode[n].college);
				puts("请输入修改后的住址：");
				char num4[MIN];
				scanf("%s",num4);
				strcpy(num4,LNode[n].adss);
				puts("请输入修改后的联系方式：");
				char num5[MIN];
				scanf("%s",num5);
				strcpy(num5,LNode[n].phone);			
			}
		}
		if(t==2){
			n = 0;
...
void PX (){
	...
	while(flag){
		...
		strcpy(wqs,pxu[number].shu);
		strcpy(wqs,pxu[number].coll);
		number++;		
		puts("是否继续录入：  1.是 2.否");
		scanf("%d",&n);
		if(n==1){
	...
	for(r=1; r<=number; r++) {
		for(t=0; t<=number-r; t++) {
			if(strcmp(pxu[t+1].shu,pxu[t].shu)>0) {//由高到低 
				temp = pxu[t+1];
				pxu[t+1]=pxu[t];
				pxu[t]=temp;
			}
