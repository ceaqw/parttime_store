...
int DelItem(ListNode *L,char *str)
{
	...
	//要删除的不是头节点
		...
		ListNode *node = head;
		while (node->next != NULL && strcmp(node->next->name,str)!=0){
			node = node->next;
		}
		if (node->next != NULL && strcmp(node->next->name,str)==0){
		...
		else if(cnt=='1')
		{
			while(1)
			{
				...
				else if(cnt=='1')
				{
					...
					{
						if(strcmp(run_msg[i_number[5]].run_number,run_msg[i].run_number)==0)
						{
							puts("已存在！！！");
							flag = 1; 
							break;
